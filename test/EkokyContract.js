...
contract("EkokyContract", (accounts) => {
    ...
    it("get offers List", async() => {
        ...
        assert.equal(offer.objective, "We waste fruit, vegetables and canned food in good condition");
        assert.equal(offersCounter, 1);
    });

    it("offer created successfully", async() => {
        const result = await this.ekokyContract.createOffer(2, "A restaurant", "Total organic waste in bad condition", "Total organic waste in bad condition", "Valladolid #1337 Mexico", "restaurant@gmail.com", "012 3223 23 44");
        const offerEvent = result.logs[0].args;
        const offersCounter = await this.ekokyContract.offersCounter();

        assert.equal(offersCounter, 2);
        assert.equal(offerEvent.id.toNumber(), 2);
        assert.equal(offerEvent.name, "A restaurant");
        assert.equal(offerEvent.objective, "Total organic waste in bad condition");
    });
